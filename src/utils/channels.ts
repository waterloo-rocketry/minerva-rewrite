...
export function filterSlackChannelFromName(
  ...
  if (name == 'default')
    throw new Error(
      '`default` is not a valid channel name, as it is a group of channels',
    );
  const channel = channels?.find((channel) => channel.name === name);
  if (channel == undefined)
    throw new Error(`Could not find channel with name ${name}`);

  return channel;
}

export function filterSlackChannelsFromNames(
  names: string[],
  channels: SlackChannel[],
): SlackChannel[] {
  ...
  for (const name of names) {
    if (name == 'default') {
      filterDefaultSlackChannels(channels).forEach((channel) =>
        res.add(channel),
      );
    } else {
      const channel = channels.find((channel) => channel.name === name);
...
export function filterDefaultSlackChannels(
  channels: SlackChannel[],
): SlackChannel[] {
